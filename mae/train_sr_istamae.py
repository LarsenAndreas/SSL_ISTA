...
import models_sr_istamae as istamae
...
class params:
    ...
    embed_depth = pars.get('embed_depth')
...
model = istamae.ISTAMAESR(pf.img_size_hr, img_size=p.img_size, patch_size=p.patch_size, in_chans=p.in_chans,
...
folder = f"output/{timestamp}_mmaesr_finetuning"
os.mkdir(folder)
...
pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'decoder' not in k} # decoder_pred
...
model.to(device)
print(f"ISTA-MAE SR Model Settings:\n{pformat(p.model_params(p))}")
...
for epoch in range(1, pf.epochs + 1):
    ...
    for batch_HR in train_loader:
        ...
        loss, y = model(batch_LR, batch_HR, alpha=pf.alpha)
        loss_mean += loss.item()
        loss.backward()
        optimiser.step()
